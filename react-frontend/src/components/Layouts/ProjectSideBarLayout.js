...
const ProjectSideBarLayout = (props) => {
  ...
  const matchesConfig = (cfg, ctx) => {
    // base fields (arrays)
    const rolesOk =
      !cfg.roles || cfg.roles.length === 0 || (ctx.roleId && cfg.roles.includes(ctx.roleId));
    const positionsOk =
      !cfg.positions ||
      cfg.positions.length === 0 ||
      (ctx.positionId && cfg.positions.includes(ctx.positionId));
    const profilesOk =
      !cfg.profiles ||
      cfg.profiles.length === 0 ||
      (ctx.profileId && cfg.profiles.includes(ctx.profileId));
    ...
    const deptOk =
      !cfg.department || (ctx.departmentId && String(cfg.department) === String(ctx.departmentId));
